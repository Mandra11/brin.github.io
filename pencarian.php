...
<table border="1">
 ...
 if(isset($_GET['cari'])){
  $cari = $_GET['cari'];
  $data = mysqli_query($conn,"select * from data_arsip where uraian_informasi_arsip like '%".$cari."%'");    
 }else{
...
<table border="1">
 ...
 if(isset($_GET['cari'])){
  $cari = $_GET['cari'];
  $data = mysqli_query($conn,"select * from data_arsip_2 where uraian_informasi_arsip_2 like '%".$cari."%'");    
 }else{
  $data = mysqli_query($conn,"select * from data_arsip_2");  
 }
 $no = 1;
 while($d = mysqli_fetch_array($data)){
 ?>
